...
Reaction。同一规则的条件可以使用 AND（全部命中）或 OR（任一命中）组合，
每个 A / B / C 条件都可以独立选择下列匹配类型：
...
- `单条件（A）`：A 命中；
- `全部匹配（A AND B）`：A 和 B 都命中；
- `任一匹配（A OR B）`：A 或 B 任一命中；
- `任一匹配（A OR B OR C）`：A、B 或 C 任一命中；
- `全部匹配（A AND B AND C）`：A、B 和 C 都命中。
    ...
    "rules": [
      {
        ...
        "reactions": ["👍"],
        "match_mode": "all",
        "condition_a": {"match_type": "keyword", "pattern": "部署完成"},
        "condition_b": {"match_type": "group_id", "pattern": "!ci:example.org"}
      },
      {
        "__template_key": "a_or_b_or_c",
        "selection": "random",
        "reactions": ["👍", "🎉"],
        "match_mode": "any",
        "condition_a": {"match_type": "keyword", "pattern": "build passed"},
        "condition_b": {"match_type": "keyword", "pattern": "deploy success"},
        "condition_c": {"match_type": "user_id", "pattern": "@ci:example.org"}
      },
      {
